...
public interface PromotionRepository extends JpaRepository<Promotion, Integer> {
    ...
    @Query("SELECT COUNT(p) FROM Promotion p WHERE (:startDate BETWEEN p.startDate AND p.endDate " +
            "OR :endDate BETWEEN p.startDate AND p.endDate) " +
            "AND ( :promotionId IS NULL OR p.id <> :promotionId ) " +
            "AND p.isActive = true " +
            "AND p.deletedAt IS NULL")
    int countOverlappingPromotions(@Param("promotionId") Long promotionId, @Param("startDate") LocalDate startDate,
